...
On fetch success → updates cache and posts a message to the Textual app.
On fetch failure → serves stale cache, sets from_cache=True on result.
On new/changed alert → triggers notification callback.
"""
...
from api import nws, open_meteo
...
class Scheduler:
    ...
    async def _fetch_conditions(self) -> None:
        if not self._coords or not self._client:
            return
        try:
            conditions, indicators = await open_meteo.fetch_conditions_and_indicators(
                self._coords.lat, self._coords.lon, self._client
            )
            forecast = await open_meteo.fetch_forecast(
                self._coords.lat, self._coords.lon, self._client
            )
